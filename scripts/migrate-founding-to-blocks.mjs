...
// Convert plain strings to minimal Portable Text blocks
const blocks = texts.map((text, i) => ({
  _type: 'block',
  _key: `founding-para-${i}`,
  style: 'normal',
  markDefs: [],
...
const result = await client
  ...
  .set({ 'founding.paragraphs': blocks })
  .commit()

console.log('✓ Founding paragraphs migrated to Portable Text blocks:', result._id)
