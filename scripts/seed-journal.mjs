/**
 * Seed script — Journal Articles
 * Usage: SANITY_TOKEN=<write-token> node scripts/seed-journal.mjs
 */
...
// Convert "12 Mar 2025" → "2025-03-12"
...
// Map frontend body blocks → Sanity schema blocks
// Note: 'image' blocks are skipped — cover images must be uploaded manually in Studio
function mapBody(blocks) {
  return blocks
    .filter(b => b.type !== 'image') // images require Sanity asset upload
    .map(b => {
      if (b.type === 'p')       return { _type: 'paragraph',    _key: key(), type: 'p',       text: b.text }
      if (b.type === 'h2')      return { _type: 'heading',      _key: key(), type: 'h2',      text: b.text }
      if (b.type === 'quote')   return { _type: 'quote',        _key: key(), type: 'quote',   text: b.text, attribution: b.attribution ?? '' }
      if (b.type === 'divider') return { _type: 'divider',      _key: key(), type: 'divider' }
      return null
    })
    .filter(Boolean)
}
...
const ARTICLES = [
  {
    ...
    body: [
      ...
      { type:'image', src:'', caption:"Namche Bazaar at dawn — the acclimatisation hub of the Khumbu Valley." },
  ...
  },
  {
    ...
    body: [
      ...
      { type:'p', text:"What most expedition clients don't know is that a team called the Icefall Doctors — Sherpa route-fixers employed collectively by expedition companies — rebuilds the route through the icefall every single season. They place the aluminium ladders across crevasses. They fix the ropes. They move the route around newly formed hazards." },
      ...
      { type:'image', src:'', caption:"Fixed ropes and ladder crossings in the Khumbu Icefall — rebuilt every season by the Icefall Doctors." },
  ...
  },
  {
    ...
    authorTitle: 'High Altitude Specialist · MOI Certified',
    date: '14 Feb 2025',
    readTime: '6 min read',
    featured: false,
    body: [
  ...
  },
  {
    ...
    authorTitle: 'Route & Safety Director · IFMGA / MOI',
    date: '01 Feb 2025',
    ...
    body: [
      ...
      { type:'image', src:'', caption:"Trail simulation training — loaded pack, elevation gain, consecutive days." },
    ],
  },
  {
  ...
  {
    slug: 'sherpa-kitchen-trail-food',
    title: 'Inside the Sherpa Kitchen: What We Eat on the Trail',
    excerpt: "Dal bhat twice a day isn't tradition — it's performance nutrition. We break down the caloric science behind the Sherpa diet and why our guides eat the same meal every evening.",
    category: 'Community',
    tags: ['Nutrition', 'Sherpa Culture', 'Trail Food', 'Community'],
    author: 'Dawa Gyalje Sherpa',
    authorTitle: 'High Altitude Specialist · MOI Certified',
    date: '05 Jan 2025',
    readTime: '5 min read',
    ...
    body: [
      ...
      { type:'p', text:"A 2018 study on Sherpa physiology found that traditional Sherpa diets — high in complex carbohydrates, moderate in protein, low in saturated fat — correlate with better SpO₂ maintenance at altitude compared to Western high-protein diets. This is not surprising to anyone who has spent significant time in the Khumbu." },
  ...
  },
  {
    ...
    body: [
      ...
      { type:'image', src:'', caption:"Markha Valley in high summer — empty trails and peak wildflower bloom." },
    ],
  },
]
...
async function run() {
  ...
  for (const a of ARTICLES) {
    const doc = {
      ...
      readTime:    a.readTime,
      body:        mapBody(a.body),
      // image: must be uploaded manually in Studio (cover image)
    }

    const result = await client.createOrReplace(doc)
    console.log(`  ✓ ${result._id} — ${a.title.slice(0, 50)}`)
  }

  console.log('\nDone. Note: cover images and body images must be uploaded manually in Studio.')
}
