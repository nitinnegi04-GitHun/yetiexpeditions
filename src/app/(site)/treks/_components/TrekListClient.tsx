...
export default function TrekListClient({ treks }: Props) {
          ...
          {filtered.length === 0 ? (
            <div className="py-16 text-center border-b border-zinc-border">
              <p className="text-sm text-slate-400 uppercase tracking-widest font-bold">
                No expeditions found for this filter.
              </p>
            </div>
          ) : (
            filtered.map((trek, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-12 mt-6 md:mt-0">
                  ...
                  <span className="text-xl font-bold tracking-tight">
                    {trek.investment} <span className="text-[10px] text-slate-400 uppercase">USD</span>
                  </span>
        ...
        {(activeRegion !== 'All' || activeDifficulty !== 'All') && (
          <div className="mt-6 flex items-center gap-4 flex-wrap">
            <p className="text-[10px] text-slate-400 uppercase tracking-widest">
              Showing{' '}
              <span className="text-slate-900 font-black">{filtered.length}</span>{' '}
              expedition{filtered.length !== 1 ? 's' : ''}
              {activeRegion !== 'All' && (
                <> in <span className="text-slate-900 font-black">{activeRegion}</span></>
              )}
              {activeDifficulty !== 'All' && (
                <> · <span className="text-slate-900 font-black">{activeDifficulty}</span></>
              )}
            </p>
            <button
