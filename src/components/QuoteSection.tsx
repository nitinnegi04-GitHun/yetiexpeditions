...
export default function QuoteSection({ data }: { data?: QuoteData }) {
  const quote       = data?.quote       ?? "The mountains are not a place to conquer, but a place to rediscover what it means to be human under the strict guidance of nature."
  const author      = data?.author      ?? "Reinhold Messner"
  const authorTitle = data?.authorTitle ?? "Alpine Legend"
