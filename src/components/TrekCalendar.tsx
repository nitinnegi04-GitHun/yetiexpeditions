...
export default function TrekCalendar({ treks }: { treks: Trek[] }) {
    ...
    const monthTreks = treks.flatMap(trek => {
        ...
        if (!batch) return [];
        const effectiveLead = batch.trekLead ?? null;
                                ...
                                {monthTreks.map((trek, index) => (
                                    <div key={trek.slug} className={`group hover:bg-slate-50 transition-colors ${index !== monthTreks.length - 1 ? "border-b border-zinc-border" : ""}`}>
                                        ...
                                        <div className="trek-card-row items-stretch p-5 md:p-6 lg:p-0">
                                            ...
                                            {/* Col 1 — Region, Name, Dates, Special Departure */}
                                            ...
                                            {/* Col 2 — Special Departure */}
                                            <div className={`flex flex-col justify-center lg:px-6 ${trek.effectiveLead ? 'border-zinc-border/50 border-t pt-3 mb-3 lg:border-t-0 lg:pt-0 lg:mb-0 lg:border-l' : 'lg:border-l-0'}`}>
                                                {trek.effectiveLead && (
                                                    <>
                                                        <div className="flex items-center gap-1.5 mb-1">
                                                            ...
                                                            <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Special Departure</p>
                                                        </div>
                                                        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-3">This trek will be led by</p>
                                                        <div className="flex items-center gap-2.5">
                                                            {trek.effectiveLead.imageUrl ? (
                                                                ...
                                                                <div className="w-12 h-12 rounded-full bg-slate-200 shrink-0 flex items-center justify-center">
                                                                    <span className="text-[11px] font-black text-slate-500">{trek.effectiveLead.name.charAt(0)}</span>
                                                                </div>
                                                            )}
                                                            <div className="min-w-0">
                                                                <p className="text-xs font-black uppercase tracking-tight text-slate-900 leading-tight truncate">{trek.effectiveLead.name}</p>
                                                                {trek.effectiveLead.summits && (
                                                                    <p className="text-[9px] font-bold uppercase tracking-wider text-primary leading-tight mt-0.5">{trek.effectiveLead.summits}</p>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </>
                                            ...
                                            {/* Mobile stats row */}
                                            <div className="lg:hidden flex justify-between w-full border-t border-zinc-border/50 pt-3 mb-3">
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <p className={`text-[10px] font-bold uppercase ${trek.difficulty === "Difficult" ? "text-primary" : "text-amber-600"}`}>{trek.difficulty}</p>
                                                </div>
                                                <div>
                                            ...
                                            {/* Col 2 — Stats + Price + CTA in single row — desktop only */}
                                            <div className="flex max-lg:hidden items-center gap-6 justify-between pl-8 pr-6 border-l border-zinc-border/50">
                                                <div>
                                                    <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Duration</p>
                                                    <p className="text-sm font-bold uppercase">{trek.duration}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Difficulty</p>
                                                    <p className={`text-sm font-bold uppercase ${trek.difficulty === "Difficult" ? "text-primary" : "text-amber-600"}`}>{trek.difficulty}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Altitude</p>
                                                    <p className="text-sm font-bold uppercase">{trek.altitude}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Booked</p>
                                                    <p className="text-sm font-bold uppercase">{trek.activeBatch.booked} / {trek.activeBatch.capacity}</p>
                                                </div>
                                                <span className={`text-xs font-black uppercase tracking-widest px-3 py-1.5 border shrink-0 ${STATUS_STYLES[trek.activeBatch.status]}`}>
                                                    {trek.activeBatch.status}
                                                </span>
                                                ...
                                                <Link
                                                    ...
                                                    className="inline-flex items-center px-6 py-3.5 bg-zinc-900 text-white text-xs font-black uppercase tracking-widest hover:bg-primary transition-colors group/link shrink-0"
                                                >
                                                    View
